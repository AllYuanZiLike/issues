<template>
  <div class="mod-conference__baseroom">
    <el-form :inline="true" :model="dataForm" @keyup.enter="getDataList()">
      ...
      <!--      </el-form-item>-->
      <el-form-item>
        <!--        <el-input v-model="dataForm.status" placeholder="会议室状态" clearable></el-input>-->
...
<!--      </el-form-item>-->
      <el-form-item>
        <el-button v-if="hasPermission('conference:baseroom:save')" type="primary" @click="addOrUpdateHandle()">{{ $t("add") }}</el-button>
      </el-form-item>
      <el-form-item>
        <el-button v-if="hasPermission('conference:baseroom:delete')" type="danger" @click="deleteHandle()">{{ $t("deleteBatch") }}</el-button>
      </el-form-item>
    </el-form>
    <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%">
      ...
      <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          <el-button v-if="hasPermission('conference:baseroom:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t("update") }}</el-button>
          <el-button v-if="hasPermission('conference:baseroom:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t("delete") }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive({
      getDataListURL: "/conference/baseroom/page",
      getDataListIsPage: true,
      exportURL: "/conference/baseroom/export",
      deleteURL: "/conference/baseroom",
      deleteIsBatch: true,
      id:"",
      dataForm: {
        name: "",
