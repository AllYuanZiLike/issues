<template>
  <el-dialog v-model="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item label="学制年限" prop="lengthSchooling">
        <el-input v-model="dataForm.lengthSchooling" placeholder="学制年限"></el-input>
      </el-form-item>
...
export default defineComponent({
  ...
  methods: {
    ...
    // 获取信息
    getInfo() {
      baseService.get("/conference/basemajor/" + this.dataForm.id).then((res) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle() {
      ...
      that.$refs["dataFormRef"].validate((valid: boolean) => {
        ...
        (!this.dataForm.id ? baseService.post : baseService.put)("/conference/basemajor", this.dataForm).then((res) => {
          if (res.code !== 0) {
            return this['$message'].error(res.msg);
          }
          this['$message']({
